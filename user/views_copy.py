...
def load_booking_data(request, token=None):
    token = token if token is not None else request.GET.get('token', '')
    if not token:
        return {}, ''
    try:
        booking_data = signing.loads(
            token,
            max_age=BOOKING_TOKEN_MAX_AGE
        )
        return booking_data, token
    except signing.BadSignature:
        return {}, ''


def build_token_url(url_name, token):
    if not token:
        return reverse(url_name)
    return f"{reverse(url_name)}?{urlencode({'token': token})}"
...
class UserBookingView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            base_fare = 50
            ride_charge = ride_charges.get(ride_type, 180)
            platform_fee = 20
            total_amount = base_fare + ride_charge + platform_fee
            now = timezone.localtime()
            booking_data = {
                'booking_id': f"NR{now.strftime('%Y%m%d%H%M%S')}",
                ...
                'total_spent': 0,
                'fare': total_amount,
                'estimate_fare': total_amount,
                'your_rating': '',
                'driver_name': 'Raj Kumar',
                'driver_phone': '9876543210',
                'driver_rating': '4.9',
                'vehicle': vehicles.get(ride_type, 'Nandi Vehicle'),
            ...
            booking_token = signing.dumps(booking_data)
            return render(
                request,
...
class UserPaymentView(View):
    def get(self, request):
        booking_data, booking_token = load_booking_data(request)
        return render(
            request,
            'user/profile/payment.html',
            {
    ...
    def post(self, request):
        booking_token = request.POST.get('booking_token', '').strip()
        if not booking_token:
            messages.error(
                request,
                'Booking information is missing. Please book a ride again.'
            )
        ...
        try:
            ...
            messages.error(
                request,
                'Booking information is invalid or expired. Please book again.'
            )
            return redirect('booking')
        ...
        booking_data.update({
            ...
            'status': 'Confirmed',
            'total_spent': booking_data.get('total_amount', 0),
        })
        new_token = signing.dumps(booking_data)
        messages.success(
            request,
            'Payment completed successfully!'
        )
        return redirect(
            f"{reverse('myride')}?{urlencode({'token': new_token})}"
        )
    ...
    def get(self, request):
        ...
        return render(
            ...
            'user/profile/myride.html',
            {
                ...
                'booking_data': booking_data,
                'booking_token': booking_token,
                'ride_count': 1 if booking_data else 0,
                'site_name': 'NandiRide',
            }
        )
...
class UserDashboardView(View):
    def get(self, request):
        booking_data, booking_token = load_booking_data(request)
        status = booking_data.get(
            'status',
            booking_data.get('ride_status', '')
        )
        paid = booking_data.get('payment_status') == 'Paid'
        dashboard_stats = {
            'total_rides': 1 if booking_data else 0,
            'completed': 1 if status == 'Completed' else 0,
            'total_spent': booking_data.get('total_amount', 0) if paid else 0,
            'rating': booking_data.get('your_rating') or booking_data.get('driver_rating', '4.9'),
        }
        recent_rides = []
        if booking_data:
            recent_rides.append({
                'pickup': booking_data.get('pickup_location'),
                'destination': booking_data.get('destination'),
                'booking_id': booking_data.get('booking_id'),
                'date': booking_data.get('ride_date'),
                'time': booking_data.get('ride_time'),
                'vehicle': booking_data.get('ride_type'),
                'fare': booking_data.get('total_amount'),
                'status': status or 'Pending',
                'driver_name': booking_data.get('driver_name'),
...
class TrackRideView(View):
    def get(self, request):
        ...
        if not booking_data:
            messages.info(
                request,
                'Please open Track Ride from your active booking.'
            )
            return redirect('booking')
        ride = {
            'booking_id': booking_data.get('booking_id'),
            'pickup': booking_data.get('pickup_location'),
            'destination': booking_data.get('destination'),
            'ride_type': booking_data.get('ride_type'),
            'date': booking_data.get('ride_date'),
            'time': booking_data.get('ride_time'),
            'passengers': booking_data.get('passengers'),
            'distance': f"{booking_data.get('distance', '0')} km",
            'fare': booking_data.get('total_amount', 0),
            'status': booking_data.get('status', 'Pending'),
            'driver_name': booking_data.get('driver_name'),
            'driver_rating': booking_data.get('driver_rating', '4.9'),
            'driver_phone': booking_data.get('driver_phone'),
            'vehicle': booking_data.get('vehicle'),
            'vehicle_number': booking_data.get('vehicle_number'),
            'eta': booking_data.get('eta', '10 min'),
            'remaining_distance': booking_data.get(
                'remaining_distance',
                '2.4 km'
            ),
        }
...
class CancelRideView(View):
    def post(self, request):
        booking_token = request.POST.get(
            'booking_token',
            ''
        ).strip()
        if not booking_token:
        ...
        try:
            ...
            messages.error(
                request,
                'Booking information is invalid or expired.'
            )
            return redirect('booking')
        booking_data['status'] = 'Cancelled'
        booking_data['ride_status'] = 'Cancelled'
        if booking_data.get('payment_status') == 'Paid':
            booking_data['payment_status'] = 'Refund Pending'
        new_token = signing.dumps(booking_data)
        messages.success(
            request,
...
class HelpChatbotView(View):
    def post(self, request):
        ...
        if any(
            ...
            reply = 'Hello! 👋 Welcome to NandiRide Help. How can I help you today?'
        elif 'book' in message or 'booking' in message:
            reply = 'To book a ride, open the Booking page, enter your pickup location, destination, ride type, date and time, then confirm your booking.'
        elif 'payment' in message or 'pay' in message:
            reply = 'You can complete your ride payment from the Payment page. After successful payment, your ride status will be updated to Confirmed.'
        elif (
            'track' in message
            or 'where' in message
            ...
            if booking_data:
                reply = f"Your current ride status is {booking_data.get('status', 'Pending')}. You can open My Rides or Track Ride for more details."
            else:
                reply = 'You do not have an active booking yet. Please book a ride first.'
        elif 'cancel' in message:
            if booking_data:
                reply = 'You can cancel your current ride from the Cancel Ride option on your My Ride page.'
            else:
                reply = 'You do not have an active ride to cancel.'
        elif (
            'booking id' in message
            or message == 'id'
        ):
            booking_id = booking_data.get('booking_id')
            if booking_id:
                reply = f'Your booking ID is {booking_id}.'
            else:
                reply = 'You do not have an active booking yet.'
        elif (
            'contact' in message
            or 'support' in message
            or 'help' in message
        ):
            reply = 'You can contact NandiRide support through the Contact page or email support@nandiride.com.'
        else:
            reply = 'Sorry, I did not understand that. You can ask me about booking, payment, tracking, cancellation, booking ID or support.'
        return JsonResponse({
            'reply': reply
        })